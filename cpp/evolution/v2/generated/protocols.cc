...
bool ProtocolWithChangesIndexedReaderBase::ReadStreamIntToStringToFloat(float& value, size_t idx) {
  return ReadStreamIntToStringToFloatImpl(value, idx);
}

bool ProtocolWithChangesIndexedReaderBase::ReadStreamIntToStringToFloat(std::vector<float>& values, size_t idx) {
  if (values.capacity() == 0) {
    throw std::runtime_error("vector must have a nonzero capacity.");
  }
...
bool ProtocolWithChangesIndexedReaderBase::ReadIntToUnionStream(std::variant<std::string, int32_t>& value, size_t idx) {
  return ReadIntToUnionStreamImpl(value, idx);
}

bool ProtocolWithChangesIndexedReaderBase::ReadIntToUnionStream(std::vector<std::variant<std::string, int32_t>>& values, size_t idx) {
  if (values.capacity() == 0) {
    throw std::runtime_error("vector must have a nonzero capacity.");
  }
  if (!ReadIntToUnionStreamImpl(values, idx)) {
    return values.size() > 0;
  }
  return true;
}

size_t ProtocolWithChangesIndexedReaderBase::CountIntToUnionStream() {
  return CountIntToUnionStreamImpl();
}

bool ProtocolWithChangesIndexedReaderBase::ReadUnionStreamTypeChange(std::variant<int32_t, float>& value, size_t idx) {
  return ReadUnionStreamTypeChangeImpl(value, idx);
}

bool ProtocolWithChangesIndexedReaderBase::ReadUnionStreamTypeChange(std::vector<std::variant<int32_t, float>>& values, size_t idx) {
  if (values.capacity() == 0) {
    throw std::runtime_error("vector must have a nonzero capacity.");
  }
  if (!ReadUnionStreamTypeChangeImpl(values, idx)) {
    return values.size() > 0;
  }
  return true;
}

size_t ProtocolWithChangesIndexedReaderBase::CountUnionStreamTypeChange() {
  return CountUnionStreamTypeChangeImpl();
}
...
bool ProtocolWithChangesIndexedReaderBase::ReadAddedRecordStream(evo_test::RecordWithChanges& value, size_t idx) {
  return ReadAddedRecordStreamImpl(value, idx);
}

bool ProtocolWithChangesIndexedReaderBase::ReadAddedRecordStream(std::vector<evo_test::RecordWithChanges>& values, size_t idx) {
  if (values.capacity() == 0) {
    throw std::runtime_error("vector must have a nonzero capacity.");
  }
  if (!ReadAddedRecordStreamImpl(values, idx)) {
    return values.size() > 0;
  }
  return true;
}

size_t ProtocolWithChangesIndexedReaderBase::CountAddedRecordStream() {
  return CountAddedRecordStreamImpl();
}

bool ProtocolWithChangesIndexedReaderBase::ReadAddedUnionStream(std::variant<evo_test::RecordWithChanges, evo_test::RenamedRecord>& value, size_t idx) {
  return ReadAddedUnionStreamImpl(value, idx);
}

bool ProtocolWithChangesIndexedReaderBase::ReadAddedUnionStream(std::vector<std::variant<evo_test::RecordWithChanges, evo_test::RenamedRecord>>& values, size_t idx) {
  if (values.capacity() == 0) {
    throw std::runtime_error("vector must have a nonzero capacity.");
  }
  if (!ReadAddedUnionStreamImpl(values, idx)) {
    return values.size() > 0;
  }
  return true;
}

size_t ProtocolWithChangesIndexedReaderBase::CountAddedUnionStream() {
  return CountAddedUnionStreamImpl();
}
