...
func WriteProtocols(ns *dsl.Namespace, st dsl.SymbolTable, packageDir string) error {
	...
	for _, p := range ns.Protocols {
		if err := writeAbstractWriter(p, ns, st, packageDir); err != nil {
			return err
		}
		if err := writeAbstractReader(p, ns, packageDir); err != nil {
			return err
		}
	}
...
func writeAbstractWriter(p *dsl.ProtocolDefinition, ns *dsl.Namespace, st dsl.SymbolTable, packageDir string) error {
	b := bytes.Buffer{}
	...
	definitionsPath := path.Join(packageDir, fmt.Sprintf("%s.m", common.AbstractWriterName(p)))
	return iocommon.WriteFileIfNeeded(definitionsPath, b.Bytes(), 0644)
}

func writeAbstractReader(p *dsl.ProtocolDefinition, ns *dsl.Namespace, packageDir string) error {
	b := bytes.Buffer{}
	w := formatting.NewIndentedWriter(&b, "  ")
	common.WriteGeneratedFileHeader(w)
	...
	definitionsPath := path.Join(packageDir, fmt.Sprintf("%s.m", common.AbstractReaderName(p)))
	return iocommon.WriteFileIfNeeded(definitionsPath, b.Bytes(), 0644)
}
