...
func EvolutionChangeTypeSyntax(tc dsl.TypeChange) string {
	return TypeSyntax(tc.OldType())
	// switch tc := tc.(type) {
	// // TODO: Does this need to be recursive? Nested definition changes might result in "old and removed" names permitted in the type syntax??
	// case *dsl.TypeChangeDefinitionChanged:
	// 	return TypeSyntax(tc.NewType())
	// default:
	// 	x := TypeSyntax(tc.OldType())
	// 	// log.Error().Msgf("TypeChange %T: %v", tc, tc)
	// 	return x
	// 	// return TypeSyntax(tc.OldType())
	// }
}
