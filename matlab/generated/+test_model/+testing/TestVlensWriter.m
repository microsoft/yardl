...
classdef TestVlensWriter < test_model.VlensWriterBase
  ...
  methods
    function self = TestVlensWriter(testCase, format, create_writer, create_reader)
      self.filename_ = tempname();
      self.format_ = format;
      self.writer_ = create_writer(self.filename_);
      self.create_reader_ = create_reader;
      self.mock_writer_ = test_model.testing.MockVlensWriter(testCase);
  ...
  methods (Access=protected)
    function write_int_vector_(self, value)
      self.writer_.write_int_vector(value);
      self.mock_writer_.expect_write_int_vector_(value);
    end

    function write_complex_vector_(self, value)
      self.writer_.write_complex_vector(value);
      self.mock_writer_.expect_write_complex_vector_(value);
    end

    function write_record_with_vlens_(self, value)
      self.writer_.write_record_with_vlens(value);
      self.mock_writer_.expect_write_record_with_vlens_(value);
    end

    function write_vlen_of_record_with_vlens_(self, value)
      self.writer_.write_vlen_of_record_with_vlens(value);
      self.mock_writer_.expect_write_vlen_of_record_with_vlens_(value);
    end
