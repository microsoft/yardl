...
classdef RoundTripTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testEnums(testCase, format)
            ...
            RE = @test_model.RecordWithEnums;
            RN = @test_model.RecordWithNoDefaultEnum;
            w.write_rec_array([...
                    RE(enum=test_model.Fruits.PEAR, rec=RN(enum=test_model.Fruits.BANANA)), ...
                    RE(enum=test_model.Fruits.APPLE, rec=RN(enum=test_model.Fruits.APPLE)) ...
                ]);

            SR = @test_model.SimpleRecord;
            RV = @test_model.RecordWithVlens;
            RFV = @test_model.RecordWithFixedVectors;
            w.write_rec_with_fixed_vectors_array([...
                    RFV(fixed_int_vector=int32([1, 2, 3, 4, 5]), ...
                        fixed_simple_record_vector=[SR(x=1, y=2, z=3), SR(x=4, y=5, z=6), SR(x=7, y=8, z=9)], ...
                        fixed_record_with_vlens_vector=[RV(a=[SR(x=1, y=2, z=3)], b=-7, c=22), RV(a=[SR(x=4, y=5, z=6), SR(x=7, y=8, z=9)], b=11, c=13)]), ...
                    RFV(fixed_int_vector=int32([6, 7, 8, 9, 10]), ...
                        fixed_simple_record_vector=[SR(x=10, y=11, z=12), SR(x=13, y=14, z=15), SR(x=16, y=17, z=18)], ...
                        fixed_record_with_vlens_vector=[RV(a=SR.empty(), b=0, c=0), RV(a=[SR(x=2, y=3, z=4)], b=1, c=2)]) ...
                ]);

            ROF = @test_model.RecordWithOptionalFields;
            w.write_rec_with_optional_fields_array([...
                    ROF(optional_int=int32(5)), ...
                    ROF(optional_int_alternate_syntax=int32(9)) ...
                ]);

            w.write_rec_with_vlens_array([...
                    RV(a=[SR(x=1, y=2, z=3)], b=-7, c=22), ...
                    RV(a=[SR(x=4, y=5, z=6), SR(x=7, y=8, z=9)], b=11, c=13) ...
                ]);

            RS = @test_model.RecordWithStrings;
