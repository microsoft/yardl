...
classdef TestProtocolWithOptionalDateWriter < test_model.ProtocolWithOptionalDateWriterBase
  ...
  methods
    function self = TestProtocolWithOptionalDateWriter(testCase, format, create_writer, create_reader)
      self.filename_ = tempname();
      self.format_ = format;
      self.writer_ = create_writer(self.filename_);
      self.create_reader_ = create_reader;
      self.mock_writer_ = test_model.testing.MockProtocolWithOptionalDateWriter(testCase);
    ...
    function delete(self)
      ...
      if ~self.close_called_
        % ADD_FAILURE() << ...;
        throw(yardl.RuntimeError("Close() must be called on 'TestProtocolWithOptionalDateWriter' to verify mocks"));
      end
    end
  end
