...
func TestEnumAutoAssigned(t *testing.T) {
	src := `
X: !enum
  values:
    - a
...
func TestEnumSomeAssigned(t *testing.T) {
	src := `
X: !enum
  values:
    a: 1
...
`
	...
	assert.Equal(t, int64(3), e.Values[2].IntegerValue.Int64())
	assert.Equal(t, int64(-8), e.Values[3].IntegerValue.Int64())
	assert.Equal(t, int64(-9), e.Values[4].IntegerValue.Int64())
}

func TestEnumAutoAssignedConflict(t *testing.T) {
	src := `
X: !enum
  values:
    a: 0
...
`
	_, err := parseAndValidate(t, src)
	assert.ErrorContains(t, err, "in enum 'X', the symbols [b c] have the same value of 1")
}
