...
template <typename T>
constexpr bool ShouldSerializeFieldValue(std::unique_ptr<T> const& value) {
...
struct adl_serializer<std::unique_ptr<T>> {
  static void to_json(ordered_json& j, std::unique_ptr<T> const& value) {
    if (value) {
      j = *value;
    } else {
      j = nullptr;
  ...
  static void from_json(ordered_json const& j, std::unique_ptr<T>& value) {
    if (j.is_null()) {
      value = nullptr;
    } else {
      value.reset(new T{});
