...
class NDArray : public external::hoNDArray<T> {
 ...
 */
template <typename T>
class DynamicNDArray : public external::hoNDArray<T> {
  using BaseType = external::hoNDArray<T>;
...
/** Yardl NDArray free function implementation */
template <typename T>
...
std::array<size_t, N> shape(yardl::NDArray<T, N> const& arr) {
  ...
  std::copy(arr.get_dimensions().rbegin(), arr.get_dimensions().rend(), ashape.begin());
  return ashape;
}

template <typename T>
std::vector<size_t> shape(external::hoNDArray<T> const& arr) {
  return detail::reversed(arr.get_dimensions());
}

template <typename T>
size_t shape(external::hoNDArray<T> const& arr, size_t dim) {
  return arr.get_size(arr.get_number_of_dimensions() - dim - 1);
}

template <typename T>
void resize(external::hoNDArray<T>& arr, std::vector<size_t> const& shape) {
...
template <typename T>
size_t dimension(external::hoNDArray<T> const& arr) {
  return arr.get_number_of_dimensions();
}

template <typename T>
T* dataptr(external::hoNDArray<T>& arr) {
  return arr.get_data();
}
template <typename T>
T const* dataptr(external::hoNDArray<T> const& arr) {
  return arr.get_data();
}

template <typename T, class... Args>
T const& at(external::hoNDArray<T> const& arr, Args... idx) {
