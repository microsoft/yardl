...
int main(void) {
  ...
  std::complex<float> cxflt;
  r.ReadComplexFloatToComplexDouble(cxflt);
  EVO_ASSERT_EQUALISH(cxflt.real(), M_PI);
  EVO_ASSERT_EQUALISH(cxflt.imag(), -M_PI);
  ...
  while (r.ReadStreamIntToStringToFloat(int_vec)) {
    for (auto& i : int_vec) {
      EVO_ASSERT(i == 42);
    }
  }
  ...
  int_vec.clear();
  r.ReadVectorIntToStringToFloat(int_vec);
  for (auto& i : int_vec) {
    EVO_ASSERT(i == 42);
  }
